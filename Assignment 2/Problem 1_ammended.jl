...
md"""The file includes a wide range of molecular descriptors, which are numerical or categorical values that characterize the chemical properties of the compounds. These descriptors can be grouped into several categories:"""

# ╔═╡ d979bb70-b035-4429-9f61-f43d7f68d6c3
md"## Cleaning Dataset"

# ╔═╡ 8337465f-be8a-4be6-b100-222aa2c67dd4
datad = DataFrame(CSV.File("C:/Users/DPilemoni/Downloads/dia.csv", header=1))

# ╔═╡ d231c777-67eb-4952-bcfe-7ea92417921e
#Drop the column: SMILES
select!(datad, Not([:smiles,]))

# ╔═╡ 4347da21-c5fb-4f8d-ae10-0acb723d24f6
#make all column names lowercase

for col_name in names(datad)
...
datadc = dropmissing(datad)  

# ╔═╡ 37da7dcb-4b5a-490b-a73f-1cb2b8d975fb
describe(datadc)

# ╔═╡ f8141161-4119-42b6-aa12-ac5b0404140d
#Modify datadc by coercing the types of features to types suitable for modeling.
coerce!(datadc, Count => Continuous, Textual => Multiclass)

# ╔═╡ 8deda3df-1280-46f9-b620-ea2e8a2103b1
schema(datadc) |> DataFrames.DataFrame

# ╔═╡ 800e2778-f0c5-4682-8dd9-3e061d0bb260
#create a copy form datadc as to not affect the original data moving forward
copy_datadc = deepcopy(datadc)
...
for col in names(copy_datadc)
    col_data = copy_datadc[!, col]
    if eltype(col_data) <: Number && std(skipmissing(col_data)) < 1e-6
        select!(copy_datadc, Not(col))  # remove the column
    end
end
...
#First, numerical features are standardized. Then, categorical features are one-hot encoded
mach = machine(Standardizer() |> MLJ.OneHotEncoder(), copy_datadc) |> MLJ.fit!

# ╔═╡ 0d48c8de-ae02-48ff-bdc9-c150918525cb
#apply trained preprocessing pipeline (mach) to the dataset
trans = MLJ.transform(mach, copy_datadc)
...
#Preprocess data (standardize and one-hot encode it) before applying unsupervised KMeans clustering.
kmodel = Standardizer() |> MLJ.OneHotEncoder() |> KMeans(k=2)

# ╔═╡ 66ba3075-867e-4ebb-8290-055e4d866833
#creates an MLJ machine named kmach by binding kmodel to datad.
kmach = machine(kmodel, copy_datadc)

# ╔═╡ 88eadeef-a490-4ae2-ad0b-9f8bfd4adce0
#fit the full preprocessing and KMeans clustering pipeline and bound to data.
MLJ.fit!(kmach)

# ╔═╡ 7ed44a3a-85ab-4a8b-b232-c74f4eb7a11c
#extract the training report from fitted kmach, which holds useful details about the KMeans clustering result.
rpt = report(kmach)

# ╔═╡ 53979f1a-78cd-485c-b269-8faf7175a2cd
#retrieves and transposes the cluster centroids from the fitted KMeans model within kmach pipeline.
fitted_params(kmach).k_means.centers'

# ╔═╡ 80039329-6fda-4dfd-b856-8caea1cdefcc
#converts the transformed table trans into a plain matrix
matx = Matrix(trans)

# ╔═╡ 932ecfd7-afb8-49b7-bb84-97486bb76149
#computes silhouette scores for a k-means clustering result.
#The silhouette score measures how well each data point fits into its assigned cluster compared to other clusters. 
km_sil = silhouettes(rpt.k_means.assignments, matx'; metric=SqEuclidean(), batch_size=256)

# ╔═╡ 213fb11b-9f1b-4060-9f5e-7c214a5c6b2d
#Evaluate clustering 
#measure how well each data point fits within its assigned cluster:
...
#HierarchicalClustering: This specifies the clustering method to use 
#linkage = :complete: Specifies the linkage criterion to use when merging clusters.
#metric = SqEuclidean(): Defines the distance metric to use
ACmod = HierarchicalClustering(linkage = :complete, metric=SqEuclidean())
...
#create PCA model
pcamod = PCA(maxoutdim=2,variance_ratio=1)

# ╔═╡ 6fe500d6-f45d-4c92-9f29-56264767427d
#creates an machine, pcamach, which combines the pcamod with trans.
pcamach = machine(pcamod, trans)

# ╔═╡ 3edf315e-0af2-4b8c-ab37-a891343876a7
#fits  pcamach to trans
MLJ.fit!(pcamach)

# ╔═╡ a9857dec-a326-49ba-a2dc-20e1303bf5cc
#applies the PCA transformation to trans using pcamach
pcatrans = MLJ.transform(pcamach, trans)

# ╔═╡ 39204b5a-50b2-4f1f-a23e-acc73461e7bf
#creates ACmach, binding ACmod to the data pipeline.
ACmach = machine(ACmod)
...
# predict ACmach on the PCA-transformed data
pred = MLJ.predict(ACmach, pcatrans[1:end,:])

# ╔═╡ d88ef0cc-5a1c-4ec2-8551-244760078c6d
MLJ.fit!(ACmach, force = true)

# ╔═╡ 5f666c39-b03b-4616-9a4e-1068ff838ccd
#Fect ACmach report which contains important details about the hierarchical clustering process and results
rpt2 = report(ACmach)
...
#is used to cut the dendrogram from the hierarchical clustering at a specific height 
report(ACmach).cutter(h = 30)

# ╔═╡ 8eb097c7-4935-4ca2-9dcb-75fb26549220
#create new variable Xpca which is a subset of pcatrans
Xpca = pcatrans[1:end, :]

# ╔═╡ 2bc21e24-aa10-438a-86f2-f97b57266aef
# ╠═╡ disabled = true
#=╠═╡
#Create ,atrix from Xpca
X_pca_mat = Matrix(Xpca)
  ╠═╡ =#
...
data(pcatrans) * AlgebraOfGraphics.density() * mapping(:x1) |> draw()

# ╔═╡ 3e0cecd6-819a-4a50-a9be-2682f95c273d
data(pcatrans) * mapping(:x1, :x2) * visual(Scatter) |> draw()
...
#create an instance of the Gaussian Mixture Model (GMM) with 2 clusters (n_classes=2).
GMmod = GMM(n_classes=2)

# ╔═╡ c4215ce8-bb16-46e7-9365-1ff35409c15d
#create GMmach, which binds GMmod to pcatrans
GMmach = machine(GMmod, pcatrans)
...
#generate predictions from GMmach for pcatrans
GMpred = MLJ.predict(GMmach, pcatrans)

# ╔═╡ c168fe4c-f608-4f11-a63d-51fd9d3e49cc
begin
using MLJ: mode

# Now you can safely use `mode.(...)`
predicted_labels = mode.(GMpred)

cluster_counts = countmap(predicted_labels)

total_points = sum(values(cluster_counts))
cluster_proportions = Dict(k => v / total_points for (k, v) in cluster_counts)

println("Cluster counts: ", cluster_counts)
println("Cluster proportions: ", cluster_proportions)
end

# ╔═╡ ed5b908b-1398-4484-af20-0dce62dc7ea3
mean(GMpred)

# ╔═╡ 12a8e6a0-d45c-40ab-ae83-2adf2434134e
X_pca_mat = Matrix(Xpca)


# ╔═╡ 26ec6647-cf81-4027-924c-43cb0dae4cb2
#calculate the silhouette scores for the hierarchical clustering results using X_pca_mat
AC_sils = silhouettes(convert(Vector{Int}, pred), X_pca_mat'; metric=SqEuclidean(), batch_size=256)
...
# ╔═╡ a052a7d4-4f16-4c4a-b35c-50e76d124c74
begin
using CategoricalArrays

# Convert categorical cluster labels to integers
gmm_labels = levelcode.(predicted_labels)

agglo_labels = levelcode.(mode.(pred))

gmm_sil = silhouettes(gmm_labels, X_pca_mat'; metric=SqEuclidean())
end

# ╔═╡ fd9d1532-d793-4eb9-a18d-49ef0d5d203c
#retrieve the distinct cluster labels from the prediction result pred
levels(pred)

# ╔═╡ 565cb634-389e-44c4-a0ef-26d95906ea92


# ╔═╡ 9c8f73de-e122-4761-8cf3-b7f1999b62b9
...
manifest_format = "2.0"
project_hash = "41a1083e6126e6cf597f26f013bd382874958a6c"
