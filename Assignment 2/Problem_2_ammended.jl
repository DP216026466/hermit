...
# ╔═╡ 98ec9e50-29a2-11f0-1374-457c37dd4236
begin
	using PlutoUI
	using ARFFFiles
	using DataFrames
...
# ╔═╡ 1e44c8a2-44b1-41b1-a2ab-9b425098080e
using MLJBase, Plots

# ╔═╡ 5ee5fb53-b7a4-4836-a5d7-bda99316f7b8
using MLJModels

# ╔═╡ 22a6e011-46e5-479e-8cfe-87fa4372a143
#Load data
datad = ARFFFiles.load(DataFrame, "C:/Users/DPilemoni/Downloads/ag_soybean.arff")

# ╔═╡ 2cc89a46-ada5-4f6d-864c-9c0b5fcc9a50
md"# Data Preparation"

# ╔═╡ be92cbb9-70b1-47a0-9285-d71f5c47be40
datad1 = DataFrames.select(datad, [2,3,8,9,10,11,15,16,19,20,21,22,23,25,26,27,28,30,32,33,35,36])

# ╔═╡ 0ee72404-c12a-4184-959e-148a5522a7d7
coerce!(datad1, :class => Continuous, )

# ╔═╡ 51de4494-1220-4617-ab22-2891b9fcd225
schema(datad1)

# ╔═╡ 03be435a-02a0-40d8-859b-e6028bcadf90
coerce!(datad1, Textual => Multiclass)

# ╔═╡ fb13f52a-ec18-47c4-948b-42b62a3b1c8c
schema(datad1)

# ╔═╡ 78419c2d-89e0-4db3-a4c4-6996a2ce1dbf
dropmissing!(datad1)

# ╔═╡ 357dcce4-3c3b-4598-a2cd-e1e570007c12
schema(datad1)

# ╔═╡ 8ca39a30-0bad-438a-85ef-47ee064205a8
coerce!(datad1, :class => OrderedFactor,)

# ╔═╡ 5a578f88-efc8-4688-95c2-89e689c65b08
Y, X = unpack(datad1, ==(:class))

# ╔═╡ bc20635c-014c-42c2-954e-bceb5dae7c38
dmach = machine(OneHotEncoder(), X) |> MLJ.fit!

# ╔═╡ 62e247aa-fcdb-4426-9f6b-68fc3015c744
Xtrans = MLJ.transform(dmach, X)

# ╔═╡ a0c158f7-34ba-488e-82b3-9edd080287b9
train, test = partition(eachindex(Y), 0.85; rng=Xoshiro(456))

# ╔═╡ 6cac2ea7-13bc-43d6-9b89-d218c870a164


# ╔═╡ 8908023e-e3e1-46c1-a8b6-3908e10ab682
md"# Decision Tree"

# ╔═╡ 4beffd94-cf89-4e1f-87a3-b6d3276cae0e
DecisionTreeClassifier = @load DecisionTreeClassifier pkg=DecisionTree

# ╔═╡ 58df3b18-bc09-424b-9121-01085c925c9f
dtmod = DecisionTreeClassifier(max_depth=5, min_samples_split=5)

# ╔═╡ 9352410b-b6d3-402c-be47-9a43dbb544df
cndtmod = ContinuousEncoder() |> dtmod

# ╔═╡ db59f180-2361-4878-8d4f-38853ff3e31e
dtmach = machine(cndtmod, Xtrans, Y)

# ╔═╡ 213895e2-3616-4945-b4a2-208f8d33e9a0
MLJ.fit!(dtmach, rows=train)

# ╔═╡ 2e16e493-10bc-4600-993e-b9a1ef6d54e7
testdt1Y = MLJ.predict(dtmach, rows=test)

# ╔═╡ f64599cb-e227-4502-9d79-213d78ac56ec
condt = MLJ.confusion_matrix(mode.(testdt1Y), Y[test])

# ╔═╡ 1777f93e-c6a0-44ad-a69d-85849f7fd69e
MLJ.accuracy(condt)

# ╔═╡ 90f088ea-45ef-49e9-9108-edcf39fd2504
md"# Booster"

# ╔═╡ 1e44d588-e315-48ef-b1b7-0a87e92372eb
Booster = @load EvoTreeClassifier pkg=EvoTrees

# ╔═╡ 17c82ebf-0033-4662-bea6-980b8df327e5
bmod = Booster(tree_type="oblivious",rng=Xoshiro(456))

# ╔═╡ b808d805-10ae-49dd-869e-c982c5a150e2
bmod2 = ContinuousEncoder() |> bmod

# ╔═╡ 085c480d-f063-401c-940b-4bec9e766bc3
bmach = machine(bmod2, Xtrans, Y)

# ╔═╡ 4b6a2a69-1844-4dc0-8c3e-af0156d1bca8
MLJ.fit!(bmach, rows=train)

# ╔═╡ 9ee5540c-628d-454f-b59a-b6a74f36b7f8
tY = MLJ.predict(bmach, rows=test)

# ╔═╡ dafc3e52-b780-4977-b48f-03d50a3c6203
tYcon = confusion_matrix(mode.(tY), Y[test])

# ╔═╡ 45a39d34-8b52-4635-92fc-e6fb6b73c7be
MLJ.accuracy(tYcon)

# ╔═╡ bd2be896-c6aa-42d8-80f1-d2d82248d8c6
Booster2 = @load AdaBoostStumpClassifier pkg=DecisionTree

# ╔═╡ 47cbe28d-7fa2-4710-a2a9-95c8fdf3f18a
bsmod = ContinuousEncoder() |> Booster2()

# ╔═╡ 4051b3df-dd3b-4147-9769-8092c78628a2
bsmach = machine(bsmod, Xtrans, Y)

# ╔═╡ ace304d5-3e1e-4e70-9f86-16e7f571d614
MLJ.fit!(bsmach, rows=train)

# ╔═╡ 44cd4dec-4c76-40ff-b685-4a291572f17a
bstestY = MLJ.predict(bsmach, rows=test)

# ╔═╡ 520bd9dd-67d7-43df-aba4-91c7d0cc11ee
bscon = confusion_matrix(mode.(bstestY), Y[test])

# ╔═╡ 991681c1-3f03-4580-9e64-3b6850e70d4a
MLJ.accuracy(bscon)

# ╔═╡ 266eec1c-85bd-4313-9694-f0c5107a2c65
bs_roc = MLJ.roc_curve(bstestY, Y[test])

# ╔═╡ 7418d118-8c26-4009-9977-228a530f3a18
# ╠═╡ disabled = true
#=╠═╡
# Get probabilistic predictions
y_pred = MLJ.predict(dmach, rows=test)
  ╠═╡ =#

# ╔═╡ a43dceea-ed72-41dc-bc03-8f439886aeb0
begin
	# 1. Define your model (e.g., Decision Tree)
	@load DecisionTreeClassifier pkg=DecisionTree
	model = DecisionTreeClassifier()
	
	# 2. Define your preprocessing pipeline
	@load OneHotEncoder
	onehot = OneHotEncoder(; features=Symbol[])  # Empty means encode all categorical features
	
	# 3. Create a machine for the preprocessing
	mach_encoder = machine(onehot, X)
	
	# 4. Fit the encoder
	MLJ.fit!(mach_encoder)
	
	# 5. Transform your data
	X_encoded = MLJ.transform(mach_encoder, X)
	
	# 6. Now create and fit your model machine
	mach_model = machine(model, X_encoded, Y)
	MLJ.fit!(mach_model, rows=train)
	
	# 7. Make predictions (this will work now)
	y_pred = MLJ.predict(mach_model, rows=test)
end

# ╔═╡ a3e2b6b6-acb7-4002-b5a6-603ac779ed53


# ╔═╡ 48f67c79-91d4-400b-9bc4-b69297d2605b
# Get all class levels
classes = levels(Y[test])

# ╔═╡ 501f4d40-c8f4-4207-be9e-b84cd81c3598
# Initialize plot
plt = Plots.plot(title="One-vs-Rest ROC Curves", xlabel="False Positive Rate", ylabel="True Positive Rate")

# ╔═╡ 3c823939-876e-4435-9122-8e80520b9645
auc_scores = Dict{String,Float64}()

# ╔═╡ 46bd46c8-e7d5-4065-8798-1e3a1f5b918b


# ╔═╡ 872e0244-42fd-432f-9ec3-489edcaf0017
# Generate ROC for each class
for class in classes
    # Get probabilities for current class
    prob_class = [pdf(pred, class) for pred in y_pred]
    
    # Binarize the labels for this class
    y_binary = Y[test] .== class
    
    # Compute ROC curve
    roc_curve = MLJ.roc_curve(y_binary, prob_class)
    
    # Add to plot
    plot!(plt, roc_curve.fpr, roc_curve.tpr, label=string(class))
end

# ╔═╡ 40444bd8-8c80-4e9b-a1e8-e23d1ab51dff
md"# Bagging"

# ╔═╡ 197eed96-ce1f-4f90-954d-3fcbd90342ce
BaggingClassifier = @load BalancedBaggingClassifier pkg=MLJBalancing

# ╔═╡ 82720e4b-bafe-4d36-b54b-9606297b7cb2
bagmod = ContinuousEncoder() |> BaggingClassifier(model=LogisticClassifier(), T=5)

# ╔═╡ ad555aeb-056f-40d3-9938-fbe8d1572b9c
bagmach = machine(bagmod, Xtrans, Y)

# ╔═╡ 353702ae-feec-4abf-a48b-0745af5b4e4a
MLJ.fit!(bagmach, rows=train)

# ╔═╡ a267a0ee-9bc1-4044-b7b1-9848b4b7a941


# ╔═╡ 96dcd386-bd1e-470a-b39f-81f5afee4d91


# ╔═╡ 24b61a55-93bc-4e7d-a8f8-7d0597d07db4
md"# stacking"

# ╔═╡ 6bfce9bd-4a1b-4485-b6e4-a35ccead6dbe
smodel = Stack(;metalearner=LogisticClassifier(),
                resampling=CV(),
				measures=cross_entropy,
                tree_1=DecisionTreeClassifier(max_depth=5),
                tree_2=Booster(tree_type="oblivious",rng=Xoshiro(456)),
				tree_3 = Booster2(),
                xgb=RandomForestClassifier(max_depth=5, min_samples_split=6))

# ╔═╡ 42684348-7872-43e1-8de2-6ae88354ae6f
smodel2 = ContinuousEncoder() |> smodel

# ╔═╡ f773d704-196b-4f9c-b8d1-7dc873791176
smach = machine(smodel2, Xtrans, Y)

# ╔═╡ d743eb4f-214e-4a48-8d59-e8537468febe
MLJ.fit!(smach, rows=train)

# ╔═╡ 39f57fcb-fd8a-4851-a868-e4f982962049
tests = MLJ.predict(smach, rows=test)

# ╔═╡ 567663d9-59cb-46d0-8ba4-df7dd96fecac
cmst = confusion_matrix(mode.(tests), Y[test])

# ╔═╡ 71a8f5a3-8f0e-4511-84ac-1ae45d2676c2
accuracy(cmst)

# ╔═╡ d5ca32be-efff-41c4-8272-1193d20badd9
...
manifest_format = "2.0"
project_hash = "613a00b8dedcfffff2b0e759903e24752dcb7c67"
...
git-tree-sha1 = "fa2e05581f7393693de1c3a3ca79ae2ded9394ac"
