...
# ╔═╡ 0b4d15de-277a-11f0-007c-2f2d3f5ab658
begin
	using CSV
	using DataFrames
	using MLJ
...
# ╔═╡ 5a83204b-9bb6-4f65-b076-6348714240b6
#Load data
datad = ARFFFiles.load(DataFrame, "C:/Users/DPilemoni/Downloads/ag_soybean.arff")

# ╔═╡ 0442c78a-9dae-4942-8cd0-8a476b033f3a
describe(datad)

# ╔═╡ dfa77ad6-a6fa-4b95-9b2c-a6195d876447
# 1. First coerce ALL features to OrderedFactor (since they're all categorical)
# and the target to Multiclass
data_coerced = coerce(datad, autotype(datad, :few_to_finite))

# ╔═╡ 42740a32-5e85-434c-9d7f-b255a33fddf9
# Verify the scitypes
schema(data_coerced)

# ╔═╡ 0ce99ab0-e5ff-4621-8a45-9bebddf53074
begin
	# 2. Split into features (X) and target (y)
	y = data_coerced[!, :class]
	x = DataFrames.select(datad, Not(:class))

end

# ╔═╡ a5247712-0026-4d01-ba5c-9d7960a7918f
# 3. Further coerce X to a table of OrderedFactors
# (DecisionTree expects either Continuous, Count, or OrderedFactor features)
X_coerced = coerce(x, autotype(x, :few_to_finite))

# ╔═╡ 282bff1f-2151-4141-8bbc-fc413468b743
begin
...
# ╔═╡ 67822bc9-1bf2-4c99-b1ca-f05233069b28
for col in categorical_cols 
	datad[!, col] = coerce(datad[!, col], OrderedFactor)
end

# ╔═╡ 42f02b78-740b-466d-bb40-787204b46d39
vz = coerce(y, Multiclass)

# ╔═╡ 67105139-1290-46c8-9a4e-e3736fcc5abc


# ╔═╡ 119e16d2-7bd9-432d-a212-0af85a5fefe5
md"### Tree-based Models"

# ╔═╡ d0bc9209-aa82-4fbd-b2a6-ca34798eb184
DecisionTreeClassifier = @load DecisionTreeClassifier pkg=DecisionTree

# ╔═╡ 456475fd-71b9-4341-a501-b3235a76bd8a
mod = DecisionTreeClassifier(max_depth=5, min_samples_split=5)

# ╔═╡ 258b6a38-9046-4906-8920-c3480ce0e8ae
modcn = ContinuousEncoder() |> mod


# ╔═╡ 695c60b2-f44d-425c-819c-9f08cedd1750
mach = machine(modcn, X_coerced, y,)

# ╔═╡ 9dabb13a-ede4-40df-8ee3-3a84bcb0f326
MLJ.fit!(mach, rows=train) 

# ╔═╡ cb58a393-2a6a-4442-ac2f-8e7717e09792
y1tmodtd = MLJ.predict(mach, rows=train)

# ╔═╡ 0039f2f0-c43a-4e03-8663-3459da27078a
cm_dt = MLJ.confusion_matrix(mode.(y1tmodtd), y[train])

# ╔═╡ 61cb9d67-03e1-442d-8cd1-7b15ba24be88
MLJ.accuracy(cm_dt)

# ╔═╡ 1952476d-78f0-4c7b-a02d-f8b7be7fafac
Booster = @load EvoTreeClassifier pkg=EvoTrees

# ╔═╡ 273f8810-91ef-4cce-bd82-aa4812d62807
booster_mod = Booster(tree_type="oblivious",rng=Xoshiro(456))

# ╔═╡ 62a6e8b9-799a-47c1-bf30-b41bd463026f
booster_mod2 = ContinuousEncoder() |> booster_mod

# ╔═╡ 7d75ed2e-73f0-41eb-afd2-eb84fe1bac6a
booster_mach = machine(booster_mod2, X_coerced, y)

# ╔═╡ 9005fbee-cad1-4c88-b46f-c7d9d3603bf1
MLJ.fit!(booster_mach, rows=train)

# ╔═╡ f73ca38a-4011-4fe7-9291-69077df5bb9c
y2= MLJ.predict(booster_mach, rows=test)

# ╔═╡ 0a89efbd-281f-4de5-aa8a-e35aaf472382
con = confusion_matrix(mode.(y2), y[test])

# ╔═╡ 5c6dcf58-a5b9-4758-9596-39de3bff266b
MLJ.accuracy(con)

# ╔═╡ b526ac3a-bea5-4a06-b4a5-2a2af4d05453
Booster2 = @load AdaBoostStumpClassifier pkg=DecisionTree

# ╔═╡ c7edae18-f01e-4caf-83fc-5bfb8bdb02ee
boostersmod = ContinuousEncoder() |> Booster2()

# ╔═╡ 6828e3b4-910b-4246-8c41-39bc25907c4d
boostersmach = machine(boostersmod, X_coerced, y)

# ╔═╡ cb9b24e8-e5e0-4938-9e58-4603b4ac726f
MLJ.fit!(boostersmach, rows=train)

# ╔═╡ 833ae833-6471-44e1-9578-dba84d471d8a
yz = MLJ.predict(boostersmach, rows=test)

# ╔═╡ 4e9efcf8-e1df-4690-822d-47cbad37f76f
cm_bs = confusion_matrix(mode.(yz), y[test])

# ╔═╡ 77aa7261-e6a8-4ca7-a07e-d2b10906bc79
MLJ.accuracy(cm_bs)

# ╔═╡ b6fbad72-ab25-4d2a-b87a-818361390add


# ╔═╡ b4cea1ee-eed4-4e2e-a6ba-ee2ed9447785
...
manifest_format = "2.0"
project_hash = "4efe3076fc6f4605a559f97bf1d32483f55ccadd"
