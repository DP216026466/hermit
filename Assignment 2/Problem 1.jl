...
# ╔═╡ 6d86a754-0a41-11f0-1063-4fc8ce2265e7
using PlutoUI

# ╔═╡ 33f322ec-e406-4e40-b07b-94a13ecc4150
using CSV

# ╔═╡ 49e2f50d-161d-4c60-84b2-6b3161fc583f
using DataFrames

# ╔═╡ 18500ef5-1939-4443-a81f-260568994371
using MLJ

# ╔═╡ ca8e6e59-7387-408d-b9a2-920208b20548
using Statistics

# ╔═╡ 95ffdeb0-4231-464a-8bcf-ea5ed95aa827
using StatsBase

# ╔═╡ 387c75cd-0767-4e01-b4cd-501168ac56b4
using CairoMakie

# ╔═╡ 6a3a80bb-85e7-495a-b94c-c4802eefd1b4
using AlgebraOfGraphics

# ╔═╡ 22e87bbb-6fb6-427b-a3c1-7d8632d93a94
using MLJClusteringInterface

# ╔═╡ ad4a6bb1-bf18-47d4-af6d-e389014b4903
using Distances

# ╔═╡ 75eb4bd5-1d1c-46ad-9d23-1c399c4298be
using Clustering

# ╔═╡ 35a738a7-74c0-481b-b963-3db03ce6da66
using StatsPlots
...
# ╔═╡ 1dcc7e84-4133-4407-87b3-1445fa7ec08d
md"""This dataset represents a survey for airline passenger satisfaction. The metadata is available [here](https://www.kaggle.com/datasets/teejmahal20/airline-passenger-satisfaction?select=train.csv)"""

# ╔═╡ d979bb70-b035-4429-9f61-f43d7f68d6c3
md"## Cleaning Dataset"

# ╔═╡ 8337465f-be8a-4be6-b100-222aa2c67dd4
apsat_df = DataFrame(CSV.File("C:/Users/DPilemoni/Downloads/dia.csv", header=1))

# ╔═╡ d231c777-67eb-4952-bcfe-7ea92417921e
select!(apsat_df, Not([:smiles,]))

# ╔═╡ 6ba6a774-cca8-47b5-889a-bb85cfb53344
begin
	rename!(apsat_df, Symbol.(replace.(string.(names(apsat_df)), Ref(r"\[m\]"=>"_"))))
    rename!(apsat_df, Symbol.(replace.(string.(names(apsat_df)), Ref(r"\[s\]"=>"_"))))
    rename!(apsat_df, Symbol.(replace.(string.(names(apsat_df)), Ref(r"\s"=>"_"))))
end

# ╔═╡ 4347da21-c5fb-4f8d-ae10-0acb723d24f6
for col_name in names(apsat_df)
        println(lowercase(col_name))
end

# ╔═╡ 4d08d87e-b9ca-4f86-a5ba-dce3292ab626
rename!(lowercase, apsat_df)

# ╔═╡ 93d75d27-ad3d-4e91-94a0-1f271cf6ac87
    rename!(apsat_df, Symbol.(replace.(string.(names(apsat_df)), Ref(r"\/"=>"_"))))

# ╔═╡ 34516c38-e8b9-496b-a7bd-7cfd36fd4572
describe(apsat_df)

# ╔═╡ e3d716e8-78e0-4676-b2b3-6dc4f5647168
md"""From the **describe** function, only 310 observations are missing in the arival\_delay\_in\_minutes. I decided to drop the observations with missing values in this instance. Other options could be considered, for example, choose the mode value or applying a more advanced imputation method."""

# ╔═╡ df6df131-0d05-4937-9a22-1f60c7b98853
clean_df = dropmissing(apsat_df)  # without !

# ╔═╡ 331c0795-3fae-4ba8-b87a-550a4d114a3f
for col in names(apsat_df)
    if eltype(apsat_df[!, col]) <: Union{Missing, Number}
        mean_val = mean(skipmissing(apsat_df[!, col]))
        apsat_df[!, col] = coalesce.(apsat_df[!, col], mean_val)
    end
end

# ╔═╡ 4724550e-ce01-43d7-b03d-249cf80d1e41
# ╠═╡ disabled = true
#=╠═╡
dropmissing!(apsat_df)
  ╠═╡ =#

# ╔═╡ f8141161-4119-42b6-aa12-ac5b0404140d
coerce!(apsat_df, Count => Continuous, Textual => Multiclass)

# ╔═╡ 8deda3df-1280-46f9-b620-ea2e8a2103b1
schema(apsat_df) |> DataFrames.DataFrame

# ╔═╡ 800e2778-f0c5-4682-8dd9-3e061d0bb260
apsat_df_cpy = deepcopy(apsat_df)

# ╔═╡ 6dbacb55-05b6-432f-b7d2-4f8afaa3d08e
apsat_df_cpy1 = deepcopy(apsat_df)

# ╔═╡ adfcb968-dab3-45f1-a616-67e6d4159eae
#Extremely small standard deviation encountered in standardization.
#Remove low-variance columns (if they don't provide useful information)
for col in names(apsat_df_cpy)
    col_data = apsat_df_cpy[!, col]
    if eltype(col_data) <: Number && std(skipmissing(col_data)) < 1e-6
        select!(apsat_df_cpy, Not(col))  # remove the column
    end
end

# ╔═╡ 755cb0f2-7003-45d1-bc30-0f2cd6214d94
md"""The cell below will standardize the continuous columns and hot-encode the categorical ones"""

# ╔═╡ c8983b7c-e840-428d-922b-f19c61e1d805
sep_trans_mach = machine(Standardizer() |> MLJ.OneHotEncoder(), apsat_df_cpy) |> MLJ.fit!

# ╔═╡ 0d48c8de-ae02-48ff-bdc9-c150918525cb
the_X = MLJ.transform(sep_trans_mach, apsat_df_cpy)

# ╔═╡ f5289bd7-054b-48ed-ad3e-fbaf29288edd
md"## Building Model"

# ╔═╡ 51e633ea-1ab8-49e5-9534-89e92cbe9ff3
md"### K-Means Clustering"

# ╔═╡ adcc8b3e-4c2f-45fe-9942-8c44e276f1e0
KMeans = @load KMeans pkg=Clustering

# ╔═╡ d214dd79-8b3c-4780-83c5-59bcf96f2439
cls_model = Standardizer() |> MLJ.OneHotEncoder() |> KMeans(k=2)

# ╔═╡ 66ba3075-867e-4ebb-8290-055e4d866833
cls_mach = machine(cls_model, apsat_df)

# ╔═╡ 51867be3-aa1b-4d5e-a797-71c1f4d23a50
cls_mac1 = deepcopy(cls_mach)

# ╔═╡ f438ddf6-fe1f-4357-83d0-7435b8a1ef97
#Extremely small standard deviation encountered in standardization.
#Remove low-variance columns (if they don't provide useful information)
for col in names(cls_mach)
    col_data = cls_mach[!, col]
    if eltype(col_data) <: Number && std(skipmissing(col_data)) < 1e-6
        select!(cls_mach, Not(col))  # remove the column
    end
end

# ╔═╡ 88eadeef-a490-4ae2-ad0b-9f8bfd4adce0
MLJ.fit!(cls_mach)

# ╔═╡ 7ed44a3a-85ab-4a8b-b232-c74f4eb7a11c
rpt = report(cls_mach)

# ╔═╡ 53979f1a-78cd-485c-b269-8faf7175a2cd
fitted_params(cls_mach).k_means.centers'

# ╔═╡ 80039329-6fda-4dfd-b856-8caea1cdefcc
X_mat = Matrix(the_X)

# ╔═╡ 932ecfd7-afb8-49b7-bb84-97486bb76149
km_sil = silhouettes(rpt.k_means.assignments, X_mat'; metric=SqEuclidean(), batch_size=256)

# ╔═╡ 213fb11b-9f1b-4060-9f5e-7c214a5c6b2d
mean(km_sil)

# ╔═╡ b47185c7-781c-499f-bc44-36267803d399
md"""The first attempt to run the agglomerative clustering failed due to the dataaset size. I will reduce the dimensionality using PCA and then apply the agglomerative clustering again."""

# ╔═╡ b6a45b4b-2315-42ec-b074-f35c6fcfff91
md"### Agglomerative Clustering"

# ╔═╡ 32e0ff50-7e44-414a-8093-8f11ba70b8ee
HierarchicalClustering = @load HierarchicalClustering pkg=Clustering

# ╔═╡ eea2b206-7564-413e-8f29-4547c07d6e3d
cls_model2 = HierarchicalClustering(linkage = :complete, metric=SqEuclidean())

# ╔═╡ 09e65d20-3f97-4893-b116-73ef87eb0aa2
PCA = @load PCA pkg=MultivariateStats

# ╔═╡ 46b98aec-15fb-45cc-824a-3e247c819138
apsat_pca_model = PCA(maxoutdim=2,variance_ratio=1)

# ╔═╡ 6fe500d6-f45d-4c92-9f29-56264767427d
apsat_pca_mach = machine(apsat_pca_model, the_X)

# ╔═╡ 3edf315e-0af2-4b8c-ab37-a891343876a7
MLJ.fit!(apsat_pca_mach)

# ╔═╡ a9857dec-a326-49ba-a2dc-20e1303bf5cc
X_pca = MLJ.transform(apsat_pca_mach, the_X)

# ╔═╡ 39204b5a-50b2-4f1f-a23e-acc73461e7bf
cls_mach2 = machine(cls_model2)

# ╔═╡ 020dff4f-c050-402f-8452-87f9827ea664
md"""The dataset was still too large to run the agglomerative clustering in one batch. So I resorted to a partial clustering with a subset of the dataset. A complete clustering should compare the partial clusters and decide which ones to merge."""

# ╔═╡ 24fd54d1-19a5-48ab-b6cb-91e3ce0568fd
grp_part1 = MLJ.predict(cls_mach2, X_pca[1:end,:])

# ╔═╡ fd9d1532-d793-4eb9-a18d-49ef0d5d203c
levels(grp_part1)

# ╔═╡ d88ef0cc-5a1c-4ec2-8551-244760078c6d
MLJ.fit!(cls_mach2, force = true)

# ╔═╡ 5f666c39-b03b-4616-9a4e-1068ff838ccd
rpt2 = report(cls_mach2)

# ╔═╡ cfc40b98-b9bd-4bc3-aefc-0600c4568fd4
StatsPlots.plot(rpt2.dendrogram)

# ╔═╡ a6456f21-a5c3-4d63-9c7d-1a947b95fdc2
report(cls_mach2).cutter(h = 30)

# ╔═╡ 8eb097c7-4935-4ca2-9dcb-75fb26549220
X_pca_part1 = X_pca[1:end, :]

# ╔═╡ 2bc21e24-aa10-438a-86f2-f97b57266aef
X_pca_part1_mat = Matrix(X_pca_part1)

# ╔═╡ d94608b3-1535-43a2-acd7-f00212de1779


# ╔═╡ 26ec6647-cf81-4027-924c-43cb0dae4cb2
hc_sils = silhouettes(convert(Vector{Int}, grp_part1), X_pca_part1_mat'; metric=SqEuclidean(), batch_size=256)

# ╔═╡ 134d2dde-6707-4ddd-8eb7-7ae07c50475b
mean(hc_sils)

# ╔═╡ cc6cd956-9b2e-4856-ab3e-de77bfac620d
rpt2.cutter(h=30, k=3)

# ╔═╡ 3b802212-baaf-4ed3-83f8-d094806d0b21
X_pca_mat = Matrix(X_pca)

# ╔═╡ 03b88cb1-9b1c-4380-a95b-eda0d2d659d4
grp_part1v1 = hclust(Distances.pairwise(Euclidean(), X_pca_mat, X_pca_mat), linkage=:complete)

# ╔═╡ c6504844-e8d7-4c8d-8680-e274f767722c
data(X_pca) * AlgebraOfGraphics.density() * mapping(:x1) |> draw()

# ╔═╡ 3e0cecd6-819a-4a50-a9be-2682f95c273d
data(X_pca) * mapping(:x1, :x2) * visual(Scatter) |> draw()

# ╔═╡ d87192de-1058-4c5d-a60b-d8fc5f34ea37
md"""Here, we are retrying the K-Means clustering with the trasnformed dataset (see dimensionality reduction)."""

# ╔═╡ 4fbd35cd-5fee-43a6-8ee0-d1430c583867
md"### K-Means with PCA"

# ╔═╡ b67f5419-f56c-437b-a02d-b154b1b0c2e0
cls_model3 = KMeans(k=2)

# ╔═╡ fe672892-965f-4c5f-ba02-b87fae621d64
cls_mach3 = machine(cls_model3, X_pca)

# ╔═╡ e71686ba-a9e7-420c-ae0f-375e006e5440
MLJ.fit!(cls_mach3)

# ╔═╡ 55f1ffc6-c00a-4767-8117-100e672ea4a1
rpt3 = report(cls_mach3)

# ╔═╡ 76a9f2d1-9b84-4dc5-86da-50caf26892e6
km_sil2 = silhouettes(rpt3.assignments, X_pca_mat'; metric=SqEuclidean(), batch_size=256)

# ╔═╡ 25ea735b-a006-49d8-bfa2-091014da9c16
mean(km_sil2)

# ╔═╡ ded9c490-346e-42e6-96d7-2b405bd9ccc8
sil_vec = [0.5609372795051865, 0.5578557855678851, 0.5312821669496305, 0.5050685798618872, 0.5036748952380345, 0.48493272886309413, 0.5012696638175191]

# ╔═╡ 8e1a7350-8828-4b3d-bc6b-63a4aeb9ca03
sil_df = DataFrame(k = range(2..8), score = sil_vec)

# ╔═╡ a58dff68-e129-45b5-a50b-dec7ddd754aa
data(sil_df) * mapping(:k, :score) * visual(Lines) |> draw()

# ╔═╡ 50c4181a-7115-4220-b693-f15cd9054a1a
fitted_params(cls_mach3).centers

# ╔═╡ bb5ac81c-b7a5-47da-82fb-3865f1c1e47a


# ╔═╡ a5b67f2a-2c41-4090-81d8-2579e95fbbba
md"### Gaussian Mixture Model"

# ╔═╡ 085d375a-615b-4117-a3a5-d7c94e6f0655
GMM   = @load GaussianMixtureClusterer pkg = "BetaML" verbosity=0

# ╔═╡ b74e174f-8236-40a3-abd4-27536b0e9994
cls_model4 = GMM(n_classes=2)

# ╔═╡ c4215ce8-bb16-46e7-9365-1ff35409c15d
cls_mach4 = machine(cls_model4, X_pca)

# ╔═╡ 0a2cf263-bdf5-4ad3-a030-7e4cbd04ba3b
MLJ.fit!(cls_mach4)

# ╔═╡ 38548ebd-c1ee-4474-a10a-07d63844f934
predict4 = MLJ.predict(cls_mach4, X_pca)

# ╔═╡ ed5b908b-1398-4484-af20-0dce62dc7ea3
mean(predict4)

# ╔═╡ 1687a609-7837-43e0-bf33-ec727de6a0c9
md"### Density-based Clustering"

# ╔═╡ 4258ef46-cf72-4e3f-810f-e5f964ddce41
DBSCAN = @load DBSCAN pkg=Clustering

# ╔═╡ ca67409a-045f-40ad-b7bb-21a6a1febe6e
cls_model5 = DBSCAN(radius=0.3, min_cluster_size=5)

# ╔═╡ 18bdd2e0-342d-4aeb-81eb-28be6c605818
cls_mach5 = machine(cls_model5)

# ╔═╡ 862b5876-b7db-40cc-8533-520b18ca6fc4
pred5 = MLJ.predict(cls_mach5, X_pca)

# ╔═╡ 77d02733-10f0-4def-93b0-a2d427fc1341
levels(pred5)
