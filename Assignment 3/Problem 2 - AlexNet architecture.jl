...
# ╔═╡ 2e006170-3c81-11f0-09f3-0fc530b90c63
begin
    using Flux
    using Statistics
    using Random
...
# ╔═╡ e957c33a-b1ab-46de-a789-4ee62dfcbc5c
begin
	using Markdown
	using InteractiveUtils
end

# ╔═╡ fc1ed689-3462-4a19-8a82-ded25a4316a4
using PlutoUI

# ╔═╡ afc85a00-cb94-4c7b-bc8e-9006025d9349
begin
    using Flux: logitcrossentropy, trainmode, testmode
    ...
    println("✓ Loss function and optimizer defined")
    println("Loss function: ", loss)
    println("Optimizer: ", optimizer)
end

# ╔═╡ eb62f230-c20e-4a15-a03b-e7829d351eaf
TableOfContents()

# ╔═╡ 4a5ceeca-7a54-49c8-bb1e-11341b367cdf
md"# New Notebooks"

# ╔═╡ e7579bc4-a123-4e4f-ab31-7c2fb63c56b9
md"## Data"

# ╔═╡ ef74faa3-5021-487e-9c0d-6d95891456c1
begin
    # Define paths and parameters
    data_path = "C:/Users/DPilemoni/Documents/Data Science 25/TAM/archive (3)/Rice_Image_Dataset"  
    image_size = (227, 227)  
    batch_size = 32
    ...
    for (class_idx, class) in enumerate(classes)
        class_path = joinpath(data_path, class)
        for file in readdir(class_path)
            if endswith(lowercase(file), ".jpg") || endswith(lowercase(file), ".png")
                try
                    img_path = joinpath(class_path, file)
                    img = load(img_path)
                    img = imresize(img, image_size)
                    img = channelview(img)
					img = permutedims(img, (2, 3, 1)) 
                    img = Float32.(img) ./ 255.0f0
                    push!(images, img)
    ...
    labels = Flux.onehotbatch(labels, 1:num_classes)
    
    # Split into train and test (80-20)
    split_idx = floor(Int, size(images, 4) * 0.8)
    shuffle_idx = randperm(size(images, 4))
    
    train_images = images[:, :, :, shuffle_idx[1:split_idx]]
    train_labels = labels[:, shuffle_idx[1:split_idx]]
    
    test_images = images[:, :, :, shuffle_idx[split_idx+1:end]]
    test_labels = labels[:, shuffle_idx[split_idx+1:end]]
    
    # Create data loaders
    train_loader = DataLoader((data=train_images, label=train_labels), batchsize=batch_size, shuffle=true)
    test_loader = DataLoader((data=test_images, label=test_labels), batchsize=batch_size)
end

# ╔═╡ 2b319524-6e09-4ecc-87be-4fbd2a48f3b3
begin
    using CUDA
    using Printf
    ...
    model = Chain(
        ...
        Conv((11, 11), 3 => 96, stride=4, pad=0),
        x -> relu.(x),
        ...
        # Layer 6: Dense -> ReLU -> Dropout
        Dense(9216, 4096),
        x -> relu.(x),
        Dropout(0.5),
        
        # Layer 7: Dense -> ReLU -> Dropout
        Dense(4096, 4096),
        x -> relu.(x),
        Dropout(0.5),
    ...
    # GPU setup
    if has_cuda()
        @info "CUDA available - Moving model to GPU"
        model = model |> gpu
        println("Model moved to GPU")
    else
        @info "No CUDA available - Using CPU"
        println("Model remains on CPU")
    end
    
    # Test the model with dummy input
    println("\nTesting model with dummy input...")
    test_input = rand(Float32, 227, 227, 3, 1)
    if has_cuda()
        test_input = test_input |> gpu
    end
    output = model(test_input)
    println("Output shape: ", size(output))
    println("First output values: ", output[:,1][1:min(5, num_classes)])
    
    # Final confirmation
    println("\n✓ Model successfully initialized")
end

# ╔═╡ 7470d5c5-2b88-48c0-86eb-6ce41983e6f6
...
begin
    ...
    Flux.testmode!(model)  # Set model to evaluation mode
    ...
    for (x, y) in test_loader
        ...
        append!(y_pred, onecold(scores))
    end
    
    # Calculate base metrics
    accuracy = mean(y_true .== y_pred)
    cm = confusmat(length(classes), y_true, y_pred)
    
    # Calculate class-wise metrics
    metrics = []
    for class in 1:num_classes
        tp = sum((y_true .== class) .& (y_pred .== class))  # True positives
        fp = sum((y_true .!= class) .& (y_pred .== class))  # False positives
        fn = sum((y_true .== class) .& (y_pred .!= class))  # False negatives
        
        precision = tp / (tp + fp + eps())  # Add eps() to avoid division by zero
        recall = tp / (tp + fn + eps())     # Recall = Sensitivity
        f1 = 2 * (precision * recall) / (precision + recall + eps())
        
        push!(metrics, (precision=precision, recall=recall, f1=f1))
    end
    
    # Calculate macro-averages
    macro_precision = mean([m.precision for m in metrics])
    macro_recall = mean([m.recall for m in metrics])
    macro_f1 = mean([m.f1 for m in metrics])
    
    # Display results
    md"""
    ### Evaluation Metrics
    
    **Overall Accuracy**: $(round(accuracy*100, digits=2))%
    
    **Class-wise Metrics**:
    $([join(["• $(classes[i]): Precision=$(round(metrics[i].precision, digits=3)), Recall/Sensitivity=$(round(metrics[i].recall, digits=3)), F1=$(round(metrics[i].f1, digits=3))" for i in 1:num_classes], "\n")])
    
    **Macro Averages**:
    - Precision: $(round(macro_precision, digits=3))
    - Recall/Sensitivity: $(round(macro_recall, digits=3))
    - F1-score: $(round(macro_f1, digits=3))
...
# ╔═╡ 767afab7-f933-48b5-8a20-a74a920a51bd
begin
    using MLBase
    
    # Class scores
    y_score_matrix = zeros(Float32, num_classes, length(y_true))
    
    Flux.testmode!(model)
    for (i, (x, y)) in enumerate(test_loader)
        x = has_cuda() ? x |> gpu : x
        scores = model(x) |> cpu
        y_score_matrix[:, (i-1)*batch_size+1:min(i*batch_size, length(y_true))] = scores
    end
    
    # calculate ROC curve and AUC
    function calculate_roc_auc(y_true_bin, y_scores)
       
        sort_idx = sortperm(y_scores, rev=true)
        sorted_scores = y_scores[sort_idx]
        sorted_labels = y_true_bin[sort_idx]
        
       
        tps = cumsum(sorted_labels)
        fps = cumsum(.!sorted_labels)
        
        
        total_p = sum(y_true_bin)
        total_n = length(y_true_bin) - total_p
        tpr = tps ./ total_p
        fpr = fps ./ total_n
    ...
    for class in 1:num_classes
        y_true_bin = y_true .== class
        fpr, tpr, auc = calculate_roc_auc(y_true_bin, y_score_matrix[class, :])
        push!(roc_curves, (fpr, tpr))
        push!(auc_scores, auc)
    end
    
    # Create the plot
    p = plot(title="ROC Curves for Rice Classification", 
    ...
    colors = palette(:rainbow, num_classes)  # Different colors for each class
    for i in 1:num_classes
        plot!(p, roc_curves[i][1], roc_curves[i][2], 
              label="$(classes[i]) (AUC = $(round(auc_scores[i], digits=3)))",
              color=colors[i],
              linewidth=2)
    end
...
# ╔═╡ 08e547c3-7b83-485a-8977-19217d693249
md"##  Model Definition"

# ╔═╡ b59198f9-4703-456d-8a53-c809fcc6d67b
md"# Define Loss Function and Optimizer"

# ╔═╡ 64d9c075-d196-4116-b4f2-6c5e8aa25667
md"# Train Model"

# ╔═╡ 509d3562-643f-4488-9057-ae817941cac2
function train_model!(model, train_loader, test_loader, loss, optimizer, epochs)
    # Initialize variables for tracking progress
    train_losses = Float32[]
    test_losses = Float32[]
    accuracies = Float32[]
    
    # Set up the optimizer with the new Flux API
    ...
    @showprogress for epoch in 1:epochs
        ...
        for (x, y) in test_loader
            ...
            ŷ = model(x)
            test_loss += loss(ŷ, y)
            correct += sum(Flux.onecold(ŷ) .== Flux.onecold(y))
            total += size(y, 2)
        end
        avg_test_loss = test_loss / length(test_loader)
        accuracy = correct / total
        push!(test_losses, avg_test_loss)
        push!(accuracies, accuracy)
        
        # Print progress
        @printf("Epoch %d: Train Loss = %.4f, Test Loss = %.4f, Accuracy = %.2f%%\n",
                epoch, avg_train_loss, avg_test_loss, accuracy * 100)
    end
    
    # Create and display training plots
    p1 = plot(1:epochs, train_losses, label="Training Loss", xlabel="Epoch", ylabel="Loss")
    plot!(1:epochs, test_losses, label="Test Loss")
    p2 = plot(1:epochs, accuracies, label="Accuracy", xlabel="Epoch", ylabel="Accuracy")
    
    # Return results
    return (train_losses=train_losses, test_losses=test_losses, accuracies=accuracies), plot(p1, p2, layout=(2,1))
end

# ╔═╡ 60022b80-dae9-4ce6-9a0d-e777b96b5024
begin
	# Train the model for 10 epochs
	results, training_plots = train_model!(model, train_loader, test_loader, loss, optimizer, 10)
	
	# The plots will automatically display in the notebook
	# You can also access the raw data through results:
	println("Final accuracy: $(results.accuracies[end]*100)%")
end

# ╔═╡ 1c4041aa-c663-4576-a087-ed1cbfadfa31
md"# Evaluation"

# ╔═╡ e91361b2-e739-4682-863d-297315ff0c92
md"## ROC Curves"

# ╔═╡ 78aabdfa-f31f-4b1f-8d86-9a193f9af0b7
md"## Confsion Matrix"

# ╔═╡ 628cdac2-a929-45f1-bc7c-489b2cb33db9
begin
    df = DataFrame(cm, :auto)
    rename!(df, Symbol.(classes))
    insertcols!(df, 1, :Actual => classes)
    df
end

# ╔═╡ e9a4a9e0-1197-4f51-935c-8b49e6344e12
...
manifest_format = "2.0"
project_hash = "9e32bf90c86c9a01001fca2a1d40cfe3d74ebb92"
