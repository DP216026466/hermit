...
# ╔═╡ 3ac98890-410a-11f0-26f3-69dec2f94b35
begin
	using Flux
...
# ╔═╡ f5a10f96-e19c-4673-912c-9cafe8c731d6
using PlutoUI

# ╔═╡ 0ac38f43-fe14-4586-9260-f41ad4d54dac
TableOfContents()

# ╔═╡ 2a98d388-09b8-437d-bfa6-b3c822a77882
md"# Data"

# ╔═╡ 150edb02-df56-4658-b5ab-0160c5bb484e
begin
	# Load train and test data
	train_df = CSV.read("C:/Users/DPilemoni/Documents/Data Science 25/TAM/Ass3/Data/medical_tc_train.csv", DataFrame)
	test_df = CSV.read("C:/Users/DPilemoni/Documents/Data Science 25/TAM/Ass3/Data/medical_tc_test.csv", DataFrame)
...
# ╔═╡ 5bf83ecb-7598-4631-b5e5-36547c58974f
begin
    using Flux: onecold, onehotbatch, logitcrossentropy, reset!, params, Embedding, Dense, Chain, LSTM, softmax
    using WordTokenizers

    # 1. Tokenization and vocabulary building
    tokenized_train = [WordTokenizers.tokenize(lowercase(text)) for text in train_texts]
    tokenized_test = [WordTokenizers.tokenize(lowercase(text)) for text in test_texts]

    all_tokens = reduce(vcat, tokenized_train)
    ...
    function tokens_to_indices(tokens, vocab)
        [get(vocab, tok, vocab["<unk>"]) for tok in tokens]
    end
    indexed_train = [tokens_to_indices(toks, vocab) for toks in tokenized_train]
    indexed_test = [tokens_to_indices(toks, vocab) for toks in tokenized_test]

    # 3. Padding sequences
    maxlen = 100
    function pad_sequence(seq, maxlen)
        if length(seq) < maxlen
            return vcat(fill(vocab["<pad>"], maxlen - length(seq)), seq)
        else
            return seq[1:maxlen]
        end
    end

    X_train = hcat([pad_sequence(seq, maxlen) for seq in indexed_train]...) |> permutedims
    X_test = hcat([pad_sequence(seq, maxlen) for seq in indexed_test]...) |> permutedims

    # 4. Prepare labels
    y_train = onehotbatch(train_labels, unique(train_labels))
    y_test = onehotbatch(test_labels, unique(train_labels))

    # 5. Model parameters
    vocab_size = length(vocab)
    embedding_dim = 100
    hidden_dim = 64
    output_dim = length(unique(train_labels))

    # 6. Correct model definition using current Flux LSTM syntax
 model = Chain(
...
# ╔═╡ 44b996a2-4c8b-4f74-90ca-e7b6d32c8ea3
md"# Tokenize and Prepare Vocabulary"

# ╔═╡ 9fb307f5-75a9-4c11-9006-e80d4cb6b0f6
md"# Model Evaluation"

# ╔═╡ 00b13e30-1050-4281-9492-1423f8d0c260
function evaluate_model(model, X_test, y_test, train_labels; ϵ=1e-10)
    # Unique classes
    unique_classes = sort(unique(train_labels))
    n_classes = length(unique_classes)

    # Input validation
    @assert ndims(X_test) in (2, 3)
    @assert size(y_test, 1) == n_classes 

    # Reset model state
    Flux.reset!(model)

    # Convert true labels from one-hot to integer
    y_true = onecold(y_test, unique_classes)

    # Format input for model
    if ndims(X_test) == 2

        X_test_corrected = X_test'
        y_pred_probs = model(X_test_corrected)

    elseif ndims(X_test) == 3
        X_test_corrected = permutedims(X_test, (2, 3, 1)) 
        X_test_corrected = reshape(X_test_corrected, size(X_test, 2), size(X_test, 3))
        y_pred_probs = model(X_test_corrected)
    end

    # Get predicted classes
    y_pred = onecold(y_pred_probs, unique_classes)
    ...
    for (true_idx, pred_idx) in zip(y_true, y_pred)
        if 1 ≤ true_idx ≤ n_classes && 1 ≤ pred_idx ≤ n_classes
    ...
    for class in 1:n_classes
        tp = confusion_mat[class, class]
        fp = sum(@view confusion_mat[:, class]) - tp
        fn = sum(@view confusion_mat[class, :]) - tp
        tn = sum(confusion_mat) - tp - fp - fn

        precision = tp / (tp + fp + ϵ)
        recall = tp / (tp + fn + ϵ)
        f1 = 2 * precision * recall / (precision + recall + ϵ)
...
# ╔═╡ 65fbd6e2-0322-4437-a5f5-85cfebcd6af7
md"# Accuracy and CM"

# ╔═╡ 4ba5bf1b-bcba-4060-bf86-8d466b7d9c0d
begin
    results = evaluate_model(model, X_test, y_test, train_labels)
    
    # Print overall accuracy
    println("Accuracy: ", round(results.accuracy * 100, digits=2), "%\n")
    
    # Print class-wise metrics
    println("Class-wise Performance:")
    for (i, class) in enumerate(results.classes)
        m = results.class_metrics[i]
        println("• ", class, ":")
        println("  Precision:    ", round(m.precision, digits=3))
        println("  Recall/Sensitivity: ", round(m.recall, digits=3)) 
        println("  F1-score:     ", round(m.f1, digits=3))
        println()
    end
    
    # Print macro-averaged metrics
    println("Macro Averages:")
    println("• Precision:    ", round(results.macro_precision, digits=3))
    println("• Recall/Sensitivity: ", round(results.macro_recall, digits=3))
    println("• F1-score:     ", round(results.macro_f1, digits=3))
    println()
...
# ╔═╡ 9af3e1fb-c8e5-452d-ae15-c27218e25e4f
begin
	...
	function calculate_roc(y_true_bin, y_scores)
	    ...
	    for thresh in thresholds
	        ...
	        push!(tpr, sum(pred_pos .& y_true_bin)/sum(y_true_bin))
	        push!(fpr, sum(pred_pos .& .!y_true_bin)/sum(.!y_true_bin))
	    end
	    
	    # Add (0,0) and (1,1) points
	    pushfirst!(tpr, 0.0); pushfirst!(fpr, 0.0)
	    push!(tpr, 1.0); push!(fpr, 1.0)
	    
	    auc = trapezoidal_integration(fpr, tpr)
	...
	begin
	    ...
	    plt = plot(
	        ...
	        legend = :bottomright,
	        size = (800, 650),  # Slightly taller for better aspect ratio
	        grid = true,
	        minorgrid = true,
	        titlefontsize = 12,
	        legendfontsize = 9
	    ...
	    colors = palette(:rainbow, length(classes))
	    
	    # Calculate and plot each curve
	    auc_scores = Float64[]
	    for (i, class) in enumerate(classes)
	        y_true_bin = y_true .== class
	        roc = calculate_roc(y_true_bin, y_probs[i, :])
	        push!(auc_scores, roc.auc)
	        
	        plot!(plt, 
	              roc.fpr, roc.tpr,
	              label = "$class (AUC = $(round(roc.auc, digits=3))",
	              linewidth = 2.5,
	              color = colors[i],
	              alpha = 0.8)
	    end
	    ...
	    annotate!(0.62, 0.08, text("Mean AUC = $(round(mean_auc, digits=3))", 10, :left, :bottom, :black))
	    
	    # Force display and save
	    display(plt)
	    savefig(plt, "roc_curve.png")  # Optional: save to file
	    plt  # Return the plot object
	end
end

# ╔═╡ d3e90e74-3dff-4ba7-830e-2964a96873e3
md"## ROC Curve"
...
manifest_format = "2.0"
project_hash = "d3f72db8049977e455fe55979b029ceb375467ad"
