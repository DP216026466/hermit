...
# ╔═╡ 9c346440-1533-11f0-2774-7361da412e23
using PlutoUI

# ╔═╡ 6f6a7727-ad49-4114-82d0-f41156fb85d2
using CSV

# ╔═╡ cc25579c-474f-4066-b49e-aa7bb2980c12
using DataFrames

# ╔═╡ 4b5a152d-8a17-44a9-83dd-a04d59c30069
using MLJ

# ╔═╡ 3f6b9ca7-6644-4028-8847-759add763903
using Statistics

# ╔═╡ 4372906d-a7ea-4afe-80a8-1ef0470cbd17
using StatsBase

# ╔═╡ 34d64104-cd96-4cba-939d-0b4b26af1816
using CairoMakie

# ╔═╡ 08653f0e-8072-4ecb-92e7-70adc7e6572c
using AlgebraOfGraphics

# ╔═╡ 805c5753-92c2-4f5d-8066-c801795b014a
using Random

# ╔═╡ a1721a56-293d-44c4-9310-e8bc3770cb9a
using MLJLinearModels

# ╔═╡ 83c4abd1-5b69-4b81-ba34-97d812f1dcca
using MLJLIBSVMInterface

# ╔═╡ 479a71ad-77fc-44fe-a982-38b892c7dd2f
using LIBSVM

# ╔═╡ 10c41abf-985c-4528-af7f-4fa22805aa83
using NearestNeighborModels

# ╔═╡ 91071def-ff0d-41ef-b5bb-2605f974dab6
using EvoTrees

# ╔═╡ 7ab157ff-8043-427c-b063-d5613ffc96eb
PlutoUI.TableOfContents()

# ╔═╡ c71f4f44-ff72-43f4-add4-bc2912c97fd2
md"### Data Exploration"

# ╔═╡ f1b3b5e6-da60-44b6-af08-7c7b14eb331c
df = DataFrame(CSV.File("C:/Users/DPilemoni/Documents/Data Science 25/TAM/Assignment/fetal_health.csv", header=1))

# ╔═╡ 5e47ac97-1b25-4ab0-9c28-c9177c753320
data(df) * mapping(:mean_value_of_long_term_variability, stack=:fetal_health, color=:fetal_health) * histogram(bins=range(0,60, 40)) |> draw()

# ╔═╡ 26697995-e8ee-4998-b8b0-52d43d050a9d
schema(df) |> DataFrames.DataFrame

# ╔═╡ 0f1108da-2050-450f-8ace-5c6bedbb3fd7
describe(df)

# ╔═╡ 9ea2745b-4269-4d89-ac4d-1374b98c285d
coerce!(df, Textual => Multiclass)

# ╔═╡ 9a8083c5-82b5-419e-be84-012a483124cd
coerce!(df, :fetal_health => OrderedFactor)

# ╔═╡ 7068ed14-5194-4c40-8970-2b58f292bc3f
schema(df) |> DataFrames.DataFrame

# ╔═╡ 17807b51-e679-4b42-bff4-a1d7fb9ab8a3
md"### Data Preparation"

# ╔═╡ ceae7f48-c499-4b98-9bdc-20d82463de7d
Y, X_raw = unpack(df, ==(:fetal_health), colname -> true)

# ╔═╡ 4ab489e7-6665-4fc4-9632-c9b8f9235c12
X_fs_mach = machine(Standardizer(), X_raw)

# ╔═╡ e03f15de-8d9c-4184-85f8-21cedc591f2b
X_norm = MLJ.transform(MLJ.fit!(X_fs_mach), X_raw)

# ╔═╡ c2105a60-26f1-4c1e-92cf-1031390fba92
henc_mach = machine(OneHotEncoder(), X_norm) |> MLJ.fit!

# ╔═╡ e9fcb30b-78d4-4e72-9e40-d3db059f43e4
X_fin = MLJ.transform(henc_mach, X_norm)

# ╔═╡ ad1db5a6-d0a7-49b0-9f00-bd8f1f19d765
schema(X_fin) |> DataFrames.DataFrame

# ╔═╡ d882e561-98b9-4e57-9648-3b88936b391c
train_set, test_set = partition(eachindex(Y), 0.85; rng=Xoshiro(456))

# ╔═╡ d1d46925-42f7-4cdb-9317-980942b54f9d
for m in models(matching(X_fin, Y))
	println(rpad(m.name, 40), rpad(m.prediction_type, 20), "($(m.package_name))")
end

# ╔═╡ db3246b7-6f8c-4da0-a1c9-1355034b5780
md"### Models"

# ╔═╡ 450645a3-5b77-4597-a80c-6a1622e169ec
md"#### Logistic Regression"

# ╔═╡ ba6cc9d0-272d-442c-9f61-6ed8886d7503
LogClassifier = @load LogisticClassifier pkg=MLJLinearModels

# ╔═╡ e5530767-1685-4b4a-b242-b0593ff10f1d
log_cla_model = LogClassifier(lambda=0.02, fit_intercept=false)

# ╔═╡ 7d9fbaa9-3f1b-49db-ad46-c14e2206d8a6
piped_log_cla_model = ContinuousEncoder() |> log_cla_model

# ╔═╡ cf2546dc-54cc-41b0-b387-a2e3bb7885a7
piped_log_cla_mach = machine(piped_log_cla_model, X_fin, Y)

# ╔═╡ 508f0c4e-d69c-40e9-9e62-7566ab35587f
MLJ.fit!(piped_log_cla_mach, rows=train_set)

# ╔═╡ 18b3684d-9e3d-490a-ab2e-3e16d698bdde
Ŷ_t1 = MLJ.predict(piped_log_cla_mach, rows=test_set) 

# ╔═╡ 1a31e68f-b96c-41a2-aec1-0929cc1ca93a
cm1 = confmat(mode.(Ŷ_t1), Y[test_set])

# ╔═╡ a72a491e-2812-4658-899a-dcd8540609f5
accuracy(cm1)

# ╔═╡ 06bd7beb-1567-4ad0-aab9-64bd8c861bd4
md"#### Support Vector Machine"

# ╔═╡ 655ba84b-6520-491f-9592-899928143e3d
SVC = @load SVC pkg=LIBSVM

# ╔═╡ 0b038c91-ee92-4091-93ba-f1d48f09e76e
svm_cla_model1 = MLJLIBSVMInterface.SVC(kernel=LIBSVM.Kernel.RadialBasis)

# ╔═╡ 08ee948c-a635-49d3-950a-d434d15ba36a
piped_svm_cla_model1 = ContinuousEncoder() |> svm_cla_model1

# ╔═╡ 88933184-c3ce-4220-9a7f-63a32a55b791
piped_svm_cla_mach1 = machine(piped_svm_cla_model1, X_fin, Y)

# ╔═╡ c6c81aeb-4032-4a75-94d3-7ad21eb50964
MLJ.fit!(piped_svm_cla_mach1, rows=train_set)

# ╔═╡ 76a20e9e-962a-41fa-bf16-b38893c315a2
Ŷ_t2 = MLJ.predict(piped_svm_cla_mach1, rows=test_set)

# ╔═╡ 8a85f1d3-522d-42cd-93bc-f79881ebce56
cm2 = ConfusionMatrix()(Ŷ_t2, Y[test_set])

# ╔═╡ 2dcfd95e-258e-46d7-a453-54c3154f2fc2
accuracy(cm2)

# ╔═╡ 33d3075c-7505-4b6f-a834-ac8ce2eff1f7
svm_cla_model2 = MLJLIBSVMInterface.SVC(kernel=LIBSVM.Kernel.Polynomial)

# ╔═╡ d6f323f9-967f-4da7-94e6-bfc07239dd6c
piped_svm_cla_model2 = ContinuousEncoder() |> svm_cla_model2

# ╔═╡ b531bf58-08d7-4008-b1a8-0d6786f1b082
piped_svm_cla_mach2 = machine(piped_svm_cla_model2, X_fin, Y)

# ╔═╡ 2b36d85c-2b54-481c-acdf-df2640bd75f6
MLJ.fit!(piped_svm_cla_mach2, rows=train_set)

# ╔═╡ 0458787e-ed9e-4580-b1cf-426d7673bf08
Ŷ_t3 = MLJ.predict(piped_svm_cla_mach2, rows=test_set)

# ╔═╡ d85cea49-689f-47fa-a2f2-d33843835973
cm3 = ConfusionMatrix()(Ŷ_t3, Y[test_set])

# ╔═╡ 4cc9e1e9-9c8d-4c36-8f70-385032e6643e
accuracy(cm3)

# ╔═╡ 0759d7d1-411c-4ba3-b12e-5366737f604b
md"#### K Nearest Neighbor"

# ╔═╡ 8ee4ad2d-41d0-49c2-83b6-95df5ed05be9
KNNClassifier = @load KNNClassifier

# ╔═╡ fb921867-ed66-43b9-8374-3649d8597cf2
knn_model = ContinuousEncoder() |> KNNClassifier(K=3)

# ╔═╡ 2b407cac-28f2-4cc0-acb3-27e94c9ce02b
knn_mach = machine(knn_model, X_fin, Y)

# ╔═╡ eafee50e-6f18-4b87-ab5f-6defef6a38f2
MLJ.fit!(knn_mach, rows=train_set)

# ╔═╡ b79ce28b-77bf-4512-898b-1ed8c0bba63d
Ŷ_t4 = MLJ.predict(knn_mach, rows=test_set)

# ╔═╡ 3e7ebc65-52e0-45b7-99b1-6dccad1ef80a
cm4 = confusion_matrix(mode.(Ŷ_t4), Y[test_set])

# ╔═╡ 621f9572-e294-4fb6-9e16-c60772f7564f
accuracy(cm4)

# ╔═╡ 5a9b970c-3f5d-4bcf-8a75-af9d15f70339
md"#### Boosted Trees"

# ╔═╡ f37919ca-f66e-4454-9a79-1fa7f9589e98
Booster = @load EvoTreeClassifier pkg=EvoTrees

# ╔═╡ 13757281-132c-4fc5-b59e-a6f5eb266a37
booster_model = Booster(tree_type="oblivious",rng=Xoshiro(456))

# ╔═╡ 124a7bc2-50f4-4647-9648-bb5fa5b45116
piped_booster_model = ContinuousEncoder() |> booster_model

# ╔═╡ b0647b60-fb75-4ca0-887b-53dfb6189b6c
piped_booster_mach = machine(piped_booster_model, X_fin, Y)

# ╔═╡ 7d71d77a-c646-4cc3-91da-2614ea939b54
MLJ.fit!(piped_booster_mach, rows=train_set)

# ╔═╡ d61646d4-82e3-4c94-828a-7b9c263b6e29
Ŷ_t5 = MLJ.predict(piped_booster_mach, rows=test_set)

# ╔═╡ 1217d5c4-c3bd-4da0-b5ce-dfcae802f91f
cm5 = confusion_matrix(mode.(Ŷ_t5), Y[test_set])

# ╔═╡ 831a2ae2-e498-4b4c-94d7-b9e2f28809f8
accuracy(cm5)

# ╔═╡ f836aaf6-1311-48d1-8448-c7cb5f44d1e2
...
manifest_format = "2.0"
project_hash = "716da745c36879cd4cdfb6e81ec7b7a779191e88"
